...
wtmp_path_log = '/root/last.log'
btmp_path_log = '/root/lastb.log'
last_ip_list = []
ip_dic = ()
try:
    ip_dic = json.load(open(btmp_path_log,'r'))
except Exception as e:
    print(e)
try:
    last_ip_list = json.load(open(wtmp_path_log,'r'))
except Exception as e:
    print(e)
print("登陆失败的IP:" , ip_dic)
print("登陆成功过的IP:" , last_ip_list)
            ...
            if size > 1:
                ...
                for ip_tmp in ip_list:
                    ...
                    if ip_dic[ip[0]] > MAX_FAIL:
                        ...
                        msg_box = ip[0] + "尝试登录了" + str(tmp_fail_time) + "次" +"\n" + ip[0][0:ip[0].rfind('.')] + '.*' + "已加入黑名单\n" + "IP地址位置 : " + ' '.join(ip_loc)
                        send_qq_mail(msg_box)
                    os.system("echo > /var/log/btmp")
                print("登陆失败ip",ip_dic)
        elif file_path == wtmp_path :
            rand_success_list = os.popen("last -n 1 | head -1").read().split(' ') 
            rand_success_list = [i for i in rand_success_list if i != '']
            if len(rand_success_list) > 5:
                if ' '.join(rand_success_list[-3:]) ==  "still logged in\n":
                    if rand_success_list[2] in last_ip_list :
                        print("用户名: %s  IP: %s成功登入\n" %(rand_success_list[0],rand_success_list[2]))
                    else :
                        last_ip_list.append(rand_success_list[2])
                        ip_loc = []
                        if check_ip(rand_success_list[2]):
                            ip_loc = get_location(rand_success_list[2])
                        msg_box = "用户名: " + rand_success_list[0] + "    IP : " + rand_success_list[2] + " 成功登入\n" + "IP地址位置 : " + ' '.join(ip_loc)
                        send_qq_mail(msg_box)
                    if ip_dic.get(rand_success_list[2]) :
                        del [ip_dic[rand_success_list[2]]]
        else :
            print("异常")
...
if __name__ == '__main__':
    ...
    try:
        ...
        monitor_obj.add_watch("/var/log/wtmp",pyinotify.IN_MODIFY)
        ...
        with open('/root/lastb.log','w+') as f:
            f.write(json_str)
        f.close()
        last_json_str = json.dumps(last_ip_list,ensure_ascii=False,indent=2) #dumps
        with open(wtmp_path_log,'w') as f:
            f.write(last_json_str)
        f.close()
        print("防火墙结束")
