...
class Arg:
    def __init__(self):
        ...
        if (
            self.arg_list[0][-3:] == ".py"
            or "/" + os.getenv("SCRIPT_TOOL_NAME") in self.arg_list[0]
        ):
            del self.arg_list[0]
        if self.isend == False:
...
class Base(Arg):
    def __init__(self, args_list: list = None,config_name="config.json"):
        ...
        self.option_dic = {}
        self.data_dir = self.tool_dir + "/data/" + Base.__subclasses__()[-1].__name__.lower() + "/"
        self.config = Json(self.data_dir + config_name)
        
        if not os.path.exists(self.config.path):
            Shell('mkdir -p '+ self.data_dir + "&& echo '{}'> %s" %(self.config.path)).exec_system()
